...
const template: TemplateRound[] = [
  ...
  { round_number: 2, name: 'Saturday PM Par 3 Scramble', round_date: '2026-11-07', session: 'PM', course_name: 'Par 3 Course', format: 'scramble', format_label: 'Scramble', holes: 9, status: 'planned' },
...
export const handler: Handler = (event) => withAdminSupabase(event, 'POST', async (supabase, body) => {
  ...
  const existingRounds = await runRows<RoundRow>(supabase.from('rounds').select('id, round_number, tee_time, status, published').eq('tour_id', tourId), 'load rounds');
  ...
  for (const round of template) {
    ...
    const row = { tour_id: tourId, ...round, tee_time: existing?.tee_time || null, notes: notes(round.session), published: existing?.published ?? false };
    if (existing) {
