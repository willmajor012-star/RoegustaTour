...
import { deleteRound, deleteTour, fetchAdminData, saveBetMarket, updateBet, saveMatch, savePlayer, saveRound, saveTour, saveTourPlayer, saveTourTeam, saveTourTeamMembers, type AdminDataResponse } from '../lib/adminApi';
...
type RoundPlanForm = { count: string; startDate: string; namePrefix: string; courseName: string; format: MatchFormat; status: Round['status'] };
...
const emptyRoundPlanForm: RoundPlanForm = { count: '3', startDate: '', namePrefix: 'Round', courseName: '', format: 'singles', status: 'planned' };
...
export function Admin() {
  ...
  const loadAdminData = async (tourId = selectedTourId) => {
    ...
    try {
      ...
      setRoundPlanForm({ ...emptyRoundPlanForm, count: nextData.rounds.length > 0 ? String(nextData.rounds.length) : '3', startDate: selected?.startDate ?? '', courseName: selected?.location ?? '' });
  ...
  const planRounds = () => {
    ...
    if (!Number.isInteger(count) || count < 1 || count > 12) {
      setSaveState('plan-rounds', { saving: false, error: 'Choose between 1 and 12 rounds.' });
      return;
    }
    void runSave('plan-rounds', 'Round plan saved.', async () => {
      const start = roundPlanForm.startDate ? new Date(`${roundPlanForm.startDate}T00:00:00`) : null;
      for (let index = 0; index < count; index += 1) {
        const roundNumber = index + 1;
        const existing = adminData?.rounds.find((round) => round.roundNumber === roundNumber);
        const date = start ? new Date(start) : null;
        if (date && start) date.setDate(start.getDate() + index);
        await saveRound({
          id: existing?.id,
          tourId: selectedTour.id,
          roundNumber,
          name: existing?.name ?? `${roundPlanForm.namePrefix || 'Round'} ${roundNumber}`,
          status: existing?.status ?? roundPlanForm.status,
          format: formatByLabel.get((existing?.formatLabel ?? '').toLowerCase()) ?? roundPlanForm.format,
          roundDate: existing?.roundDate ?? (date ? date.toISOString().slice(0, 10) : null),
          courseName: existing?.courseName ?? (roundPlanForm.courseName || null),
  ...
  const removeRound = () => {
    if (!selectedTour || !roundForm.id) return;
    if (!window.confirm(`Delete ${roundForm.name || 'this round'}? This is only allowed while it has no matches.`)) return;
    void runSave('delete-round', 'Round deleted.', async () => { await deleteRound({ id: roundForm.id as string, tourId: selectedTour.id }); setRoundForm(emptyRoundForm()); });
  };
