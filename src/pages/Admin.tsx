...
import { deleteBetMarket, deleteMatch, deleteRound, deleteTeam, deleteTour, fetchAdminData, saveBetMarket, settleBetMarket, setCurrentPublicTour, submitResult, updateBet, updateRoundPublished, updateTeamPublished, saveMatch, savePlayer, saveRound, saveTour, saveTourPlayer, saveTourTeam, saveTourTeamMembers, type AdminDataResponse } from '../lib/adminApi';
...
const tabs = ['Overview', 'Tour setup', 'Player library', 'Squads & teams', 'Rounds & tee times', 'Matches & pairings', 'Result entry', 'Bet Punto', 'Coming next'] as const;
...
type ResultForm = { roundId: string; matchId: string; pointsSideA: string; pointsSideB: string; resultText: string; published: boolean; correctionReason: string };
...
const emptyResultForm: ResultForm = { roundId: '', matchId: '', pointsSideA: '', pointsSideB: '', resultText: '', published: true, correctionReason: '' };
const resultTextHelpers = ['1UP', '2&1', '3&2', '4&3', 'AS'];
...
function roundCutoffDefault(round?: Round) {
  return round?.roundDate ? `${round.roundDate}T09:00` : '';
}
...
export function Admin() {
  ...
  const loadAdminData = async (tourId = selectedTourId) => {
    ...
    try {
      ...
      if (nextResultRound && nextResultMatch) setResultForm((current) => ({ ...current, roundId: nextResultRound.id, matchId: nextResultMatch.id, pointsSideA: current.matchId === nextResultMatch.id ? current.pointsSideA : nextResultMatch.pointsSideA === undefined ? '' : String(nextResultMatch.pointsSideA), pointsSideB: current.matchId === nextResultMatch.id ? current.pointsSideB : nextResultMatch.pointsSideB === undefined ? '' : String(nextResultMatch.pointsSideB), resultText: current.matchId === nextResultMatch.id ? current.resultText : nextResultMatch.resultText ?? '', published: nextResultMatch.published ?? true }));
    } catch (error) {
  ...
  const matchSideALabel = matchForm.sideAPlayerIds.map((playerId) => playerLabel(playersById.get(playerId))).join(' / ') || (matchForm.sideALabel || teamsById.get(matchForm.sideATeamId)?.name || 'Side A TBC');
  const matchSideBLabel = matchForm.sideBPlayerIds.map((playerId) => playerLabel(playersById.get(playerId))).join(' / ') || (matchForm.sideBLabel || teamsById.get(matchForm.sideBTeamId)?.name || 'Side B TBC');
  ...
  const saveAttendanceChanges = () => {
    ...
    void runSave('attendance-bulk', 'Attendance changes saved.', async () => {
      for (const player of attendanceChanges) {
        ...
        await saveTourPlayer({ tourId: selectedTour.id, playerId: player.id, attending: draft.attending, tourHandicap: handicap, notes: draft.notes });
      }
  ...
  const planRounds = () => {
    ...
    void runSave('plan-rounds', 'Round setup saved. Hidden Stableford markets were added for new rounds when tour players are available.', async () => {
  ...
  const createStablefordMarketsForRounds = () => {
    ...
    if (eligibleOptions.length === 0) {
      setSaveState('bet-market', { saving: false, error: 'Add attending players to this tour before creating round Stableford markets.' });
      return;
    }
    ...
    void runSave('bet-market', 'Stableford player markets created for every missing round.', async () => {
      for (const round of missingRounds) {
        await saveBetMarket({
          ...
          marketScope: 'general_pot',
          status: 'void',
          closesAt: datetimeLocalToIso(roundCutoffDefault(round)),
          resultOptionId: null,
          resultText: null,
  ...
  const submitResultEntry = (event: FormEvent<HTMLFormElement>) => {
    ...
    void runSave('result-entry', resultMatch.status === 'complete' ? 'Result corrected.' : 'Result submitted.', async () => {
      await submitResult({
        ...
        pointsSideA: Number(resultForm.pointsSideA),
        pointsSideB: Number(resultForm.pointsSideB),
  ...
  return <div className="page-stack admin-page">
    ...
    {storedSession ? <>
      ...
      {!dataLoading && !dataError && adminData ? <>
        ...
        {activeTab === 'Matches & pairings' ? <section className="card admin-panel"><p className="eyebrow">Matches & pairings</p><h3>Draft and publish pairings</h3>{!selectedTour ? <p>No tour is available.</p> : adminData.rounds.length === 0 ? <p>No rounds have been added yet.</p> : <><label>Round<select value={matchForm.roundId} onChange={(event) => { const round = adminData.rounds.find((candidate) => candidate.id === event.target.value); setMatchForm(emptyMatchForm(round, undefined, String(nextAvailableMatchNumber(round?.id ?? '', adminData.matches)))); }}><option value="">Choose round</option>{adminData.rounds.map((round) => <option value={round.id} key={round.id}>{round.name}</option>)}</select></label><div className="premium-inset"><p className="eyebrow">Sequential tee times</p><div className="admin-form-grid"><label>First tee time<input value={teeTimePlanForm.firstTeeTime} placeholder="08:10 or 9:00" onChange={(event) => setTeeTimePlanForm({ ...teeTimePlanForm, firstTeeTime: event.target.value })} /></label><label>Interval minutes<input value={teeTimePlanForm.intervalMinutes} inputMode="numeric" onChange={(event) => setTeeTimePlanForm({ ...teeTimePlanForm, intervalMinutes: event.target.value })} /></label><SaveButton state={states['tee-time-plan']} label="Apply to draft/planned matches" onClick={applySequentialTeeTimes} /></div><small>Applies to existing draft/planned matches in this round by match number. H:mm, HH:mm and TBC remain valid on individual matches.</small></div><form className="admin-form-grid" onSubmit={submitMatch}><label>Match number<input value={matchForm.matchNumber} onChange={(event) => setMatchForm({ ...matchForm, matchNumber: event.target.value })} inputMode="numeric" /></label><label>Format<select value={matchForm.format} onChange={(event) => { const format = event.target.value as MatchFormat; const maxPlayers = maxPlayersForFormat(format); setMatchForm({ ...matchForm, format, sideAPlayerIds: matchForm.sideAPlayerIds.slice(0, maxPlayers), sideBPlayerIds: matchForm.sideBPlayerIds.slice(0, maxPlayers) }); }}>{formatOptions.map((option) => <option value={option.value} key={option.value}>{option.label}</option>)}</select></label><label>Side A team<select value={matchForm.sideATeamId} onChange={(event) => { const sideATeamId = event.target.value; setMatchForm({ ...matchForm, sideATeamId, sideAPlayerIds: matchForm.sideAPlayerIds.filter((playerId) => !sideATeamId || teamIdByPlayer.get(playerId) === sideATeamId) }); }}><option value="">Choose team</option>{adminData.tourTeams.map((team) => <option value={team.id} key={team.id}>{team.name}</option>)}</select></label><label>Side B team<select value={matchForm.sideBTeamId} onChange={(event) => { const sideBTeamId = event.target.value; setMatchForm({ ...matchForm, sideBTeamId, sideBPlayerIds: matchForm.sideBPlayerIds.filter((playerId) => !sideBTeamId || teamIdByPlayer.get(playerId) === sideBTeamId) }); }}><option value="">Choose team</option>{adminData.tourTeams.map((team) => <option value={team.id} key={team.id}>{team.name}</option>)}</select></label><label>Points available<input value={matchForm.pointsAvailable} onChange={(event) => setMatchForm({ ...matchForm, pointsAvailable: event.target.value })} inputMode="decimal" /></label><label>Tee time<input value={matchForm.teeTime} onChange={(event) => setMatchForm({ ...matchForm, teeTime: event.target.value })} /></label><label>Status<select value={matchForm.status} onChange={(event) => setMatchForm({ ...matchForm, status: event.target.value as Match['status'] })}><option value="draft">Draft</option><option value="planned">Planned</option><option value="active">Active</option><option value="complete">Complete</option><option value="void">Void</option></select></label><label>Side A points<input value={matchForm.pointsSideA} onChange={(event) => setMatchForm({ ...matchForm, pointsSideA: event.target.value })} inputMode="decimal" /></label><label>Side B points<input value={matchForm.pointsSideB} onChange={(event) => setMatchForm({ ...matchForm, pointsSideB: event.target.value })} inputMode="decimal" /></label><label>Result text<input value={matchForm.resultText} onChange={(event) => setMatchForm({ ...matchForm, resultText: event.target.value })} placeholder="e.g. Team A wins 2&1" /></label><label className="publish-toggle"><input type="checkbox" checked={matchForm.published} onChange={(event) => setMatchForm({ ...matchForm, published: event.target.checked })} /> Published publicly</label><label>Side A label<input value={matchForm.sideALabel} onChange={(event) => setMatchForm({ ...matchForm, sideALabel: event.target.value })} /></label><label>Side B label<input value={matchForm.sideBLabel} onChange={(event) => setMatchForm({ ...matchForm, sideBLabel: event.target.value })} /></label><div><p className="eyebrow">Side A players</p><small>{matchForm.format === 'custom' ? 'Custom format allows flexible side sizes.' : `Choose up to ${matchMaxPlayers} player${matchMaxPlayers === 1 ? '' : 's'}.`}</small><div className="chip-list">{activePlayers.filter((player) => attendingPlayerIds.has(player.id)).map((player) => { const selected = matchForm.sideAPlayerIds.includes(player.id); const onOtherSide = matchForm.sideBPlayerIds.includes(player.id); const wrongTeam = Boolean(matchForm.sideATeamId && teamIdByPlayer.get(player.id) !== matchForm.sideATeamId); const unavailable = onOtherSide || wrongTeam; const handicap = tourPlayers.find((tourPlayer) => tourPlayer.playerId === player.id)?.tourHandicap; return <button className={`pill match-player-pill ${selected ? 'selected' : ''} ${unavailable ? 'unavailable' : ''}`} type="button" onClick={() => toggleMatchPlayer('A', player.id)} disabled={unavailable} aria-pressed={selected} title={onOtherSide ? 'Already selected for Side B' : wrongTeam ? 'Player is not assigned to the Side A team' : undefined} key={player.id}>{playerLabel(player)}{handicap !== undefined ? ` · Hcp ${handicap}` : ''}{onOtherSide ? ' · Side B' : wrongTeam ? ' · unavailable' : ''}</button>; })}</div></div><div><p className="eyebrow">Side B players</p><small>{matchForm.format === 'custom' ? 'Custom format allows flexible side sizes.' : `Choose up to ${matchMaxPlayers} player${matchMaxPlayers === 1 ? '' : 's'}.`}</small><div className="chip-list">{activePlayers.filter((player) => attendingPlayerIds.has(player.id)).map((player) => { const selected = matchForm.sideBPlayerIds.includes(player.id); const onOtherSide = matchForm.sideAPlayerIds.includes(player.id); const wrongTeam = Boolean(matchForm.sideBTeamId && teamIdByPlayer.get(player.id) !== matchForm.sideBTeamId); const unavailable = onOtherSide || wrongTeam; const handicap = tourPlayers.find((tourPlayer) => tourPlayer.playerId === player.id)?.tourHandicap; return <button className={`pill match-player-pill ${selected ? 'selected' : ''} ${unavailable ? 'unavailable' : ''}`} type="button" onClick={() => toggleMatchPlayer('B', player.id)} disabled={unavailable} aria-pressed={selected} title={onOtherSide ? 'Already selected for Side A' : wrongTeam ? 'Player is not assigned to the Side B team' : undefined} key={player.id}>{playerLabel(player)}{handicap !== undefined ? ` · Hcp ${handicap}` : ''}{onOtherSide ? ' · Side A' : wrongTeam ? ' · unavailable' : ''}</button>; })}</div></div><p className="admin-full-span match-summary"><strong>Match {matchForm.matchNumber || 'TBC'}:</strong> {matchSideALabel} v {matchSideBLabel}</p><label className="admin-full-span">Notes<textarea value={matchForm.notes} onChange={(event) => setMatchForm({ ...matchForm, notes: event.target.value })} /></label><SaveButton state={states.match} label={matchForm.id ? 'Save match changes' : 'Create match'} /></form><div className="admin-card-list">{roundMatches.length === 0 ? <p>No matches have been added for this round yet.</p> : roundMatches.map((match) => <article className="admin-mini-card" key={match.id}><div><strong>Match {match.matchNumber}: {teamsById.get(match.sideATeamId)?.name ?? 'Side A'} v {teamsById.get(match.sideBTeamId)?.name ?? 'Side B'}</strong><span>{match.format} · {match.status} · {match.published ? 'Published' : 'Draft/unpublished'} · {match.pointsAvailable} pts{match.teeTime ? ` · Tee ${formatTeeTimeDisplay(match.teeTime)}` : ''}{match.status === 'complete' ? ` · ${match.pointsSideA ?? 0}-${match.pointsSideB ?? 0}` : ''}</span></div><button type="button" onClick={() => { const participants = adminData.matchParticipants.filter((participant) => participant.matchId === match.id); setMatchForm({ ...emptyMatchForm(selectedMatchRound, match), sideAPlayerIds: participants.filter((participant) => participant.side === 'A').map((participant) => participant.playerId), sideBPlayerIds: participants.filter((participant) => participant.side === 'B').map((participant) => participant.playerId) }); }}>Edit</button>{match.status !== 'complete' ? <button type="button" onClick={() => removeMatch(match)}>Delete draft match</button> : null}{states[`delete-match-${match.id}`]?.error ? <span className="form-error">{states[`delete-match-${match.id}`]?.error}</span> : null}</article>)}</div></>}</section> : null}


        {activeTab === 'Result entry' ? <section className="card admin-panel"><p className="eyebrow">Result entry</p><h3>Submit or correct match results</h3>{!selectedTour ? <p>No tour is available.</p> : adminData.rounds.length === 0 ? <p>No rounds are available.</p> : <><form className="admin-form-grid" onSubmit={submitResultEntry}><label>Round<select value={resultRound?.id ?? ''} onChange={(event) => { const round = adminData.rounds.find((candidate) => candidate.id === event.target.value); const firstMatch = adminData.matches.find((match) => match.roundId === round?.id); setResultForm({ ...emptyResultForm, roundId: round?.id ?? '', matchId: firstMatch?.id ?? '', pointsSideA: firstMatch?.pointsSideA === undefined ? '' : String(firstMatch.pointsSideA), pointsSideB: firstMatch?.pointsSideB === undefined ? '' : String(firstMatch.pointsSideB), resultText: firstMatch?.resultText ?? '', published: firstMatch?.published ?? true }); }}><option value="">Choose round</option>{adminData.rounds.map((round) => <option value={round.id} key={round.id}>{round.name}</option>)}</select></label><label>Match<select value={resultMatch?.id ?? ''} onChange={(event) => { const match = adminData.matches.find((candidate) => candidate.id === event.target.value); setResultForm({ ...resultForm, matchId: match?.id ?? '', pointsSideA: match?.pointsSideA === undefined ? '' : String(match.pointsSideA), pointsSideB: match?.pointsSideB === undefined ? '' : String(match.pointsSideB), resultText: match?.resultText ?? '', published: match?.published ?? true }); }}><option value="">Choose match</option>{resultMatches.map((match) => <option value={match.id} key={match.id}>Match {match.matchNumber}: {teamsById.get(match.sideATeamId)?.name ?? 'Side A'} v {teamsById.get(match.sideBTeamId)?.name ?? 'Side B'}{match.status === 'complete' ? ' · complete' : ''}</option>)}</select></label><label>Side A points<input value={resultForm.pointsSideA} onChange={(event) => setResultForm({ ...resultForm, pointsSideA: event.target.value })} inputMode="decimal" /></label><label>Side B points<input value={resultForm.pointsSideB} onChange={(event) => setResultForm({ ...resultForm, pointsSideB: event.target.value })} inputMode="decimal" /></label><label className="admin-full-span">Result text<input value={resultForm.resultText} onChange={(event) => setResultForm({ ...resultForm, resultText: event.target.value })} placeholder="e.g. 2&1, 1UP or AS" /></label><div className="admin-full-span chip-list">{resultTextHelpers.map((helper) => <button className="pill" type="button" key={helper} onClick={() => setResultForm({ ...resultForm, resultText: helper })}>{helper}</button>)}</div><label className="publish-toggle"><input type="checkbox" checked={resultForm.published} onChange={(event) => setResultForm({ ...resultForm, published: event.target.checked })} /> Publish result publicly</label>{resultMatch?.status === 'complete' ? <label className="admin-full-span">Correction reason<input value={resultForm.correctionReason} onChange={(event) => setResultForm({ ...resultForm, correctionReason: event.target.value })} placeholder="What changed?" /></label> : null}<div className="admin-full-span premium-inset"><p className="eyebrow">Selected match</p>{!resultMatch ? <p>Choose a match.</p> : <><strong>{teamsById.get(resultMatch.sideATeamId)?.name ?? 'Side A'} v {teamsById.get(resultMatch.sideBTeamId)?.name ?? 'Side B'}</strong><div className="chip-list">{resultParticipants.map((participant) => <span className="pill" key={`${participant.matchId}-${participant.playerId}`}>{participant.side}: {playerLabel(playersById.get(participant.playerId))}</span>)}</div><small>Points available: {resultMatch.pointsAvailable}</small></>}</div><SaveButton state={states['result-entry']} label={resultMatch?.status === 'complete' ? 'Correct result' : 'Save result'} /></form></>}</section> : null}

        {activeTab === 'Bet Punto' ? <section className="card admin-panel"><p className="eyebrow">Bet Punto</p><h3>Markets, options and indicative payouts</h3>{!selectedTour ? <p>No tour is available.</p> : <><div className="premium-inset"><p className="eyebrow">Quick market setup</p><p>Choose a round first for day-specific markets. Stableford winner options use all attending active players; scramble and better ball options use the teams already assigned to that round's pairings.</p><div className="chip-list"><button className="pill" type="button" onClick={applyStablefordWinnerOptions}>Stableford winner: all attending players</button><button className="pill" type="button" onClick={createStablefordMarketsForRounds}>Default Stableford markets for every round</button><button className="pill" type="button" onClick={() => applyRoundTeamScoreOptions('Scramble lowest score')}>Scramble lowest score: round teams</button><button className="pill" type="button" onClick={() => applyRoundTeamScoreOptions('Better ball lowest score')}>Better ball lowest score: round teams</button></div></div><div className="bet-admin-summary"><div className="stat-grid"><div className="stat-card"><span>Tour stake log</span><strong>{formatPenceCurrency(betPuntoTotalStakePence)}</strong><small>{adminData.bets.filter((bet) => bet.status === 'active').length} active picks</small></div><div className="stat-card"><span>Settled payouts</span><strong>{formatPenceCurrency(betPuntoSettledPayoutPence)}</strong><small>Includes manual payout overrides where entered.</small></div><div className="stat-card"><span>Stableford markets</span><strong>{betPuntoStablefordSummaries.length}</strong><small>{mandatoryBettorNames.length} mandatory bettors per playing day.</small></div></div><div className="table-wrap"><table className="bet-summary-table"><thead><tr><th>Player</th><th>Picks</th><th>Staked</th><th>Settled payout</th><th>Net</th><th>W/L/P</th><th>Missing stableford</th></tr></thead><tbody>{betPuntoBettorSummaries.length === 0 ? <tr><td colSpan={7}>No Bet Punto player summary yet.</td></tr> : betPuntoBettorSummaries.map((summary) => <tr key={summary.bettorName}><td>{summary.bettorName}</td><td>{summary.totalBets}</td><td>{formatPenceCurrency(summary.totalStakePence)}</td><td>{formatPenceCurrency(summary.settledPayoutPence)}</td><td>{formatPenceCurrency(summary.netPence)}</td><td>{summary.won}/{summary.lost}/{summary.push}</td><td>{summary.missingStablefordPicks}</td></tr>)}</tbody></table></div><div className="table-wrap"><table className="bet-summary-table"><thead><tr><th>Stableford market</th><th>Status</th><th>Picks</th><th>Pot</th><th>Missing players</th></tr></thead><tbody>{betPuntoStablefordSummaries.length === 0 ? <tr><td colSpan={5}>No Stableford winner markets yet.</td></tr> : betPuntoStablefordSummaries.map((summary) => <tr key={summary.market.id}><td>{summary.market.title}</td><td>{summary.market.status}</td><td>{summary.totalBets}/{mandatoryBettorNames.length}</td><td>{formatPenceCurrency(summary.totalStakePence)}</td><td>{summary.missingBettorNames.length === 0 ? 'Complete' : summary.missingBettorNames.join(', ')}</td></tr>)}</tbody></table></div></div><form className="admin-form-grid" onSubmit={submitBetMarket}><label>Title<input value={betMarketForm.title} onChange={(event) => setBetMarketForm({ ...betMarketForm, title: event.target.value })} /></label><label>Type<select value={betMarketForm.marketType} onChange={(event) => setBetMarketForm({ ...betMarketForm, marketType: event.target.value as BetMarket['marketType'] })}><option value="player_performance">Player performance</option><option value="team_result">Team result</option><option value="match_winner">Match winner</option><option value="over_under">Over/under</option><option value="special">Special</option><option value="custom">Custom</option></select></label><label>Scope<select value={betMarketForm.marketScope} onChange={(event) => setBetMarketForm({ ...betMarketForm, marketScope: event.target.value as BetMarket['marketScope'] })}><option value="general_pot">General pot</option><option value="special">Special/manual</option></select></label><label>Status<select value={betMarketForm.status} onChange={(event) => setBetMarketForm({ ...betMarketForm, status: event.target.value as BetMarket['status'] })}><option value="open">Live / open</option><option value="closed">Closed but visible</option><option value="settled">Settled and visible</option><option value="void">Hidden / void</option></select></label><label>Round<select value={betMarketForm.roundId} onChange={(event) => setBetMarketForm({ ...betMarketForm, roundId: event.target.value })}><option value="">No round</option>{adminData.rounds.map((round) => <option value={round.id} key={round.id}>{round.name}</option>)}</select></label><label>Match<select value={betMarketForm.matchId} onChange={(event) => setBetMarketForm({ ...betMarketForm, matchId: event.target.value })}><option value="">No match</option>{adminData.matches.map((match) => <option value={match.id} key={match.id}>Match {match.matchNumber}</option>)}</select></label><label>Cutoff time<input value={betMarketForm.closesAt} onChange={(event) => setBetMarketForm({ ...betMarketForm, closesAt: event.target.value })} type="datetime-local" /></label><label>Result option<select value={betMarketForm.resultOptionId} onChange={(event) => setBetMarketForm({ ...betMarketForm, resultOptionId: event.target.value })}><option value="">No result</option>{betMarketForm.options.filter((option) => option.id).map((option) => <option value={option.id} key={option.id}>{option.label || 'Option'}</option>)}</select></label><label className="admin-full-span">Description<textarea value={betMarketForm.description} onChange={(event) => setBetMarketForm({ ...betMarketForm, description: event.target.value })} /></label><label className="admin-full-span">Result text<textarea value={betMarketForm.resultText} onChange={(event) => setBetMarketForm({ ...betMarketForm, resultText: event.target.value })} /></label><BetMarketOptionsEditor options={betMarketForm.options} activePlayers={activePlayers} teams={adminData.tourTeams} onChangeOption={updateBetOption} onChangePlayer={updateBetOptionPlayer} onRemoveOption={removeBetOption} onAddOption={addBetOption} /><SaveButton state={states['bet-market']} label={betMarketForm.id ? 'Save market' : 'Create market'} />{betMarketForm.id ? <SaveButton state={states['bet-market-settle']} label="Settle selected market" onClick={settleSelectedBetMarket} /> : null}</form><div className="admin-card-list">{adminData.betMarkets.length === 0 ? <p>No Bet Punto markets yet.</p> : adminData.betMarkets.map((market) => { const marketBets = adminData.bets.filter((bet) => bet.marketId === market.id); return <article className="admin-mini-card" key={market.id}><div><strong>{market.title}</strong><span>{market.marketScope === 'general_pot' ? 'General pot' : 'Special/manual'} · {market.status} · {marketBets.length} bets</span><div className="chip-list">{adminData.betOptions.filter((option) => option.marketId === market.id).map((option) => <span className="pill" key={option.id}>{option.label}{option.oddsDecimal ? ` · ${option.oddsDecimal}x` : ''}</span>)}</div></div><div className="admin-mini-actions"><button type="button" onClick={() => setBetMarketForm(betFormFromMarket(market))}>Edit market</button><button type="button" onClick={() => removeBetMarket(market)}>Delete market</button>{states[`delete-bet-market-${market.id}`]?.error ? <span className="form-error">{states[`delete-bet-market-${market.id}`]?.error}</span> : null}</div></article>; })}</div>{selectedBetMarket ? <div className="premium-inset"><p className="eyebrow">Bets for selected market</p>{selectedBetMarketBets.length === 0 ? <p>No bets logged.</p> : selectedBetMarketBets.map((bet) => <p key={bet.id}>{bet.bettorName} → {selectedBetMarketOptions.find((option) => option.id === bet.optionId)?.label ?? 'Option'} · {formatStakeCurrency(bet)} · {bet.status}/{bet.outcomeStatus}/{bet.payoutStatus}{bet.payoutAmountPence !== undefined ? ` · payout £${(bet.payoutAmountPence / 100).toFixed(2)}` : ''}<button type="button" onClick={() => updateBetOutcome(bet, 'won')}>Won</button><button type="button" onClick={() => updateBetOutcome(bet, 'lost')}>Lost</button><button type="button" onClick={() => updateBetOutcome(bet, 'push')}>Push</button><button type="button" onClick={() => promptBetPayout(bet)}>Payout note</button><button type="button" onClick={() => updateBetStatus(bet, bet.status === 'void' ? 'active' : 'void')}>{bet.status === 'void' ? 'Restore' : 'Void'}</button>{states[`bet-${bet.id}`]?.error ? <span className="form-error">{states[`bet-${bet.id}`]?.error}</span> : null}</p>)}</div> : null}<p className="form-success">Bet Punto tracks private tour stakes and indicative payouts only. No wallet, payment handling or money transfer is added.</p></>}</section> : null}
  ...
  return <div className="admin-full-span bet-option-builder">
    ...
    <div className="bet-option-card-grid">{options.map((option, index) => <article className="bet-option-editor-card" key={option.id ?? index}>
      ...
      <div className="bet-option-card-fields">
        ...
        <label>Match side<select value={option.linkedMatchSide} onChange={(event) => onChangeOption(index, { linkedMatchSide: event.target.value as BetOptionForm['linkedMatchSide'] })}><option value="">No side link</option><option value="A">Side A</option><option value="B">Side B</option><option value="halved">Halved</option></select></label>
