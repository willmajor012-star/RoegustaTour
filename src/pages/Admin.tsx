...
const tabs = ['Overview', 'Tour setup', 'Player library', 'Squads & teams', 'Rounds & tee times', 'Matches & pairings', 'Result entry', 'Handbook', 'Admin guide', 'Settings', 'Bet Punto', 'Coming next'] as const;
...
type RoundForm = { id?: string; roundNumber: string; name: string; roundDate: string; session: 'AM' | 'PM' | 'TBC'; courseName: string; teeTime: string; format: MatchFormat; holes: '18' | '9'; notes: string; status: Round['status'] };
...
const adminGuideSections: AdminGuideSection[] = [
  { id: 'guide-admin-access', title: 'Admin access', summary: 'Open Admin, sign in with the Admin PIN, understand Admin mode, and leave safely.', blocks: [
    ...
    { heading: 'What should happen', items: ['Before sign-in, no admin write tools are shown.', 'After sign-in, the admin section tabs appear: Overview, Tour setup, Player library, Squads & teams, Rounds & tee times, Matches & pairings, Result entry, Handbook, Admin guide, Settings, Bet Punto, and Coming next.'] },
  ...
  { id: 'guide-tour', title: 'Creating or selecting a tour', summary: 'Create annual tours, select existing tours for editing, and control the current public tour.', blocks: [
    ...
    { heading: 'Public check', items: ['Back to app, open Home, Golf, Score, Bet Punto, and More to confirm the correct current public tour is showing.', 'If the wrong tour is current public, public pages can show the wrong rosters, rounds, results, handbook, and Bet Punto markets.'] },
  ] },
  { id: 'guide-2026-format', title: '2026 format setup', summary: 'Use the 2026 helper and verify the generated rounds/prizes.', blocks: [
    { heading: 'Step-by-step', items: ['Open Rounds & tee times.', 'Use the 2026 format helper for the selected tour.', 'It creates Sat AM Faldo Course, 4BBB, 18 holes; Sat PM Par 3 Course, scramble, 9 holes; Sun Old Course, singles, 18 holes; and Mon Course TBC, 4BBB, 9 holes.'] },
  ...
  { id: 'guide-teams', title: 'Teams and rosters', summary: 'Build teams, attendance, captains, colours, and published rosters.', blocks: [
    ...
    { heading: 'Public check', items: ['Teams appear in More > Teams & players and related public team displays once teams and rosters are published.', 'If Teams & players says TBC, check attending players, team assignment, team publish flags, and current public tour.'] },
  ] },
  ...
  { id: 'guide-secondary-prizes', title: 'Secondary prize results', summary: 'Record prize winners separately from team matchplay and Bet Punto.', blocks: [
    { heading: 'What this is for', items: ['Secondary prize results are public prize outcomes such as Individual Stableford and Team lowest gross.', 'They are separate from the main team matchplay score and separate from Bet Punto markets/bets.'] },
    { heading: 'Step-by-step', items: ['Open Result entry and find Secondary prize results.', 'Use Add default 2026 prize slots to create the standard slots when needed.', 'Enter Individual Stableford winner and score, or Team lowest gross winner and score.', 'Publish or unpublish each prize result.', 'To correct a winner or score, edit the saved prize result and save again.'] },
  ...
  { id: 'guide-bet-punto', title: 'Bet Punto', summary: 'Operate markets, manual/admin bets, settlement, and reconciliation.', blocks: [
    { heading: 'What Bet Punto is / is not', items: ['Bet Punto tracks friendly picks, stakes, outcomes, and reconciliation notes.', 'It has no wallet, no payment handling, and no actual money transfer. Settlement/reconciliation is manual admin record keeping.'] },
    { heading: 'Step-by-step', items: ['Open Bet Punto.', 'Create a market with title, description, market type, scope, status, required flag, round/match links, close time, and options.', 'Use player performance markets for player outcomes, team result markets for team/match outcomes, and custom/general markets for anything else.', 'Round-linked markets should be linked to the relevant round so public visibility and summaries make sense.', 'Users place bets publicly in Bet Punto and can edit their own bet before the market closes.', 'Admin can add a missing/manual bet in the admin bet entry form for corrections; this is the manual admin bet entry workflow.', 'Close markets by changing status/close time, then select the result option and settle.', 'Use summary/reconciliation to see totals; money movement remains outside the app.', 'For a new tour, reset/archive deliberately: historical settled data can remain historical, while the selected tour can have its Bet Punto records reset when appropriate.'] },
  ...
  { id: 'guide-archive', title: 'Archiving and next year setup', summary: 'Finish a tour, preserve history, and prepare next year.', blocks: [
    { heading: 'Step-by-step', items: ['In Tour setup, set a finished tour to complete when the event is done.', 'Use archived for older tours that should remain historical.', 'Do not delete completed matches, results, rosters, handbook, or player snapshots just to start a new year.', 'Create next year’s tour with Create next tour, then build rounds, rosters, Info, and Bet Punto from a clean selected tour.'] },
    { heading: 'What remains / resets', items: ['Historical teams, rounds, matches, results, handbook, secondary prizes, and tour-specific player profiles remain with that tour.', 'Bet Punto should be reset or archived deliberately per tour; do not assume it moves money or pays users.', 'Tour-specific profiles protect historic tours from later global player edits.'] },
  ] },
...
function emptyRoundForm(round?: Round): RoundForm {
  ...
  return { id: round?.id, roundNumber: round?.roundNumber ? String(round.roundNumber) : '1', name: round?.name ?? 'Round 1', roundDate: round?.roundDate ?? '', session: noteParts.session, courseName: round?.courseName ?? '', teeTime: round?.teeTime ?? '', format: formatByLabel.get((round?.formatLabel ?? '').toLowerCase()) ?? 'singles', holes: String(round?.holes ?? 18) as '18' | '9', notes: noteParts.notes, status: round?.status ?? 'draft' };
}
...
export function Admin() {
  ...
  const submitRound = (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); if (!selectedTour) return; void runSave('round', 'Round saved. Its daily Bet Punto market was prepared when eligible options were available.', async () => { const response = await saveRound({ ...roundForm, tourId: selectedTour.id, roundNumber: Number(roundForm.roundNumber), roundDate: roundForm.roundDate || null, holes: Number(roundForm.holes) as 9 | 18, courseName: roundForm.courseName || null, teeTime: roundForm.teeTime || null, notes: combineRoundNotes(roundForm.session, roundForm.notes) }); await createDailyMarketForRound(response.round); return selectedTour.id; }); };
