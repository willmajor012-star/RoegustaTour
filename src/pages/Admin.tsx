...
const adminGuideSections = [
  { id: 'guide-start', title: 'Getting started', steps: ['Open Admin, enter the shared admin PIN, and keep your session private.', 'The stored PIN session unlocks admin tools until logout or expiry.', 'Current public tour means the tour public users see. Published/private controls whether rosters, rounds and matches appear.'], publicNote: 'Controls which tour and published data public pages can show.', warning: 'Do not set a tour current public until dates, rosters and core content are ready.' },
  { id: 'guide-tour', title: 'Creating a new tour', steps: ['Create the tour in Tour setup.', 'Set name, year, dates, location, description and status: planned, active, complete or archived.', 'Use Set current public tour only when ready.'], publicNote: 'The selected current public tour feeds public Golf, Teams and Info pages.', warning: 'Archived tours preserve history but can still be edited deliberately.' },
  { id: 'guide-teams', title: 'Creating teams and rosters', steps: ['Create named teams, set colours, captains and sort order.', 'Mark players attending, assign them to teams, then publish rosters.', 'Archived/current team displays use the tour-specific roster and team names.'], publicNote: 'Published rosters appear on public Teams and Golf team views.', warning: 'Use real team names; avoid generic labels when team names exist. If the 2026 first team is already stored as green, change its team colour here to blue; fallback blue only applies when no stored colour exists.' },
  ...
  return <div className="page-stack admin-page">
    ...
    {storedSession ? <>
      ...
      {!dataLoading && !dataError && adminData ? <>
        ...
        {activeTab === 'Admin guide' ? <section className="card admin-panel"><p className="eyebrow">Admin guide</p><h3>Operating guide</h3><p>Use this guide after PIN login. It explains what each admin area controls publicly and common mistakes to avoid.</p><div className="admin-guide-grid"><nav className="workflow-list"><a href="#guide-start">Getting started</a><a href="#guide-tour">Creating a new tour</a><a href="#guide-teams">Teams and rosters</a><a href="#guide-photos">Player profiles and photos</a><a href="#guide-rounds">Rounds and tee times</a><a href="#guide-matches">Matches and results</a><a href="#guide-info">Handbook / Info page</a><a href="#guide-bet">Bet Punto</a><a href="#guide-archive">Tour archive</a><a href="#guide-trouble">Troubleshooting</a></nav><div className="admin-guide-sections">{adminGuideSections.map((section) => <details id={section.id} key={section.id} open={section.id === 'guide-start'}><summary>{section.title}</summary>{section.steps.map((step) => <p key={step}>{step}</p>)}<small>{section.publicNote}</small>{section.warning ? <p className="form-error">Warning: {section.warning}</p> : null}</details>)}</div></div></section> : null}
