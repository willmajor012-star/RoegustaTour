...
export function Admin() {
  ...
  const betPuntoMandatorySummaries = betPuntoMarketSummaries.filter((summary) => summary.market.marketType === 'player_performance' || summary.market.marketType === 'team_result');
  ...
  const saveMarketStatus = (market: BetMarket, status: BetMarket['status']) => {
    ...
    void runSave(`bet-market-status-${market.id}`, `Bet Punto market ${status}.`, async () => {
      await saveBetMarket({ id: market.id, tourId: selectedTour.id, roundId: market.roundId ?? null, matchId: market.matchId ?? null, title: market.title, description: market.description ?? null, marketType: market.marketType, marketScope: market.marketScope, status, closesAt: market.closesAt ?? null, resultOptionId: market.resultOptionId ?? null, resultText: market.resultText ?? null, options: options.map((option) => ({ id: option.id, label: option.label, linkedPlayerId: option.linkedPlayerId ?? null, linkedTeamId: option.linkedTeamId ?? null, linkedMatchSide: option.linkedMatchSide ?? null, oddsDecimal: option.oddsDecimal ?? null, sortOrder: option.sortOrder })) });
      return selectedTour.id;
    });
  };
