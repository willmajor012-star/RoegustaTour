...
const resultOptions = ['AS', '1up', '2up', '3up', '4up', '5up', '6up', '7up', '8up', '9up', '10up', '2&1', '3&1', '3&2', '4&2', '5&3', '6&4', '7&5', '8&6', '9&7', '10&8'];
...
export function Admin() {
  ...
  const matchTitle = (match: Match) => {
    ...
    const sideA = participants.filter((participant) => participant.side === 'A').map((participant) => playerLabel(playersById.get(participant.playerId))).join(' / ') || teamsById.get(match.sideATeamId)?.name || 'Side A';
    const sideB = participants.filter((participant) => participant.side === 'B').map((participant) => playerLabel(playersById.get(participant.playerId))).join(' / ') || teamsById.get(match.sideBTeamId)?.name || 'Side B';
  ...
  const submitResultEntry = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!resultForm.resultText) { setSaveState('result-entry', { saving: false, error: 'Choose a result margin.' }); return; }
    if (resultForm.resultText !== 'AS' && !resultForm.winningSide) { setSaveState('result-entry', { saving: false, error: 'Choose Side A or Side B as the winner.' }); return; }
    const pointsSideA = resultForm.resultText === 'AS' ? 0.5 : resultForm.winningSide === 'A' ? 1 : 0;
    const pointsSideB = resultForm.resultText === 'AS' ? 0.5 : resultForm.winningSide === 'B' ? 1 : 0;
