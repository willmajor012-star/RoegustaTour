...
import { deleteBet, deleteBetMarket, deleteMatch, deleteRound, deleteTeam, deleteTour, fetchAdminData, saveBetMarket, setCurrentPublicTour, updateBet, updateRoundPublished, updateTeamPublished, saveMatch, savePlayer, saveRound, saveTour, saveTourPlayer, saveTourTeam, saveTourTeamMembers, type AdminDataResponse } from '../lib/adminApi';
...
import { compareTeeTimeValues, formatTeeTimeDisplay, normalizeTeeTime } from '../lib/display';
...
export function Admin() {
  ...
  const applySequentialTeeTimes = () => {
    ...
    const editableMatches = [...roundMatches]
      ...
      .sort((a, b) => compareTeeTimeValues(a.teeTime, b.teeTime) || a.matchNumber - b.matchNumber);
  ...
  const removeBet = (bet: Bet) => {
    if (!window.confirm(`Delete ${bet.bettorName}'s bet? This removes it from public and admin trackers.`)) return;
    void runSave(`bet-${bet.id}`, 'Bet deleted.', async () => { await deleteBet({ id: bet.id }); });
  };
