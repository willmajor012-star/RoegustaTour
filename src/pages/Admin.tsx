...
import { deleteBetMarket, resetBetPuntoTour, saveRoundPrizeResult, deleteRoundPrizeResult, createDefaultRoundPrizeResults, deleteHandbookSection, deleteItineraryItem, deleteMatch, deleteRound, deleteTeam, deleteTour, fetchAdminData, fetchPublicAccessSettings, savePublicAccessSettings, apply2026FormatTemplate, saveBet, saveBetMarket, saveHandbookSection, saveItineraryItem, settleBetMarket, setCurrentPublicTour, submitResult, updateBet, updateRoundPublished, updateTeamPublished, updateMatchPublished, saveMatch, savePlayer, saveRound, saveTour, saveTourPlayer, saveTourTeam, saveTourTeamMembers, type AdminDataResponse } from '../lib/adminApi';
...
import { formatRoundContextLabel, formatTeeTimeDisplay, normalizeTeeTime } from '../lib/display';
import { deriveMatchPoints, matchplayResultOptionsForHoles, normalizeMatchplayResult } from '../lib/matchplay';
import type { Bet, BetMarket, BetOption, Match, MatchFormat, Player, Round, Tour, TourTeam, RoundPrizeResult } from '../lib/types';
...
const tabs = ['Overview', 'Tour setup', 'Player library', 'Squads & teams', 'Courses', 'Rounds & tee times', 'Matches & pairings', 'Result entry', 'Tour itinerary', 'Admin guide', 'Settings', 'Bet Punto', 'Coming next'] as const;
type AdminTab = typeof tabs[number] | 'Handbook';
...
type TourForm = { id?: string; name: string; year: string; location: string; startDate: string; endDate: string; timezone: string; status: Tour['status']; description: string };
...
type RoundForm = { id?: string; roundNumber: string; name: string; roundDate: string; session: 'AM' | 'PM' | 'TBC'; courseId: string; courseName: string; teeTime: string; format: MatchFormat; holes: '18' | '9'; notes: string; status: Round['status'] };
type RoundPlanForm = { count: string; namePrefix: string; courseId: string; courseName: string; format: MatchFormat; status: Round['status'] };
...
type RoundPrizeForm = { id?: string; roundId: string; prizeType: RoundPrizeResult['prizeType']; title: string; winnerPlayerId: string; winnerTeamId: string; winningScoreText: string; scoreValue: string; scoreUnit: string; notes: string; linkedBetMarketId: string; published: boolean };
...
const emptyRoundPlanForm: RoundPlanForm = { count: '1', namePrefix: 'Round', courseId: '', courseName: '', format: 'singles', status: 'planned' };
...
const emptyRoundPrizeForm: RoundPrizeForm = { roundId: '', prizeType: 'individual_stableford', title: 'Individual Stableford', winnerPlayerId: '', winnerTeamId: '', winningScoreText: '', scoreValue: '', scoreUnit: 'points', notes: '', linkedBetMarketId: '', published: true };
...
const emptyItineraryForm: ItineraryForm = { itemDate: '', dayLabel: '', timeLabel: '', activity: '', location: '', notes: '', isPlaceholder: false, sortOrder: '1' };
...
type AdminGuideSection = { id: string; title: string; summary: string; blocks: AdminGuideBlock[] };

const adminGuideSections: AdminGuideSection[] = [
  { id: 'guide-admin-access', title: 'Admin access', summary: 'Open Admin, sign in with the Admin PIN, understand Admin mode, and leave safely.', blocks: [
    { heading: 'What this is for', items: ['Use this when you need PIN-protected tour setup, roster, round, match, result, itinerary, settings, or Bet Punto admin tools.'] },
    { heading: 'Step-by-step', items: ['Open Admin from the hidden logo shortcut or the existing /admin route.', 'Check the brand header so you know which public tour context the app is showing.', 'Enter an optional Admin label and the Shared PIN in the Admin PIN session card.', 'Click Create admin session. The Admin label is only audit/context text; it does not create a public user account.', 'Use Back to app to return to Overview without ending the Admin PIN session.', 'Use End admin session when you want to clear the Admin PIN session only.'] },
    { heading: 'What should happen', items: ['Before sign-in, no admin write tools are shown.', 'After sign-in, the admin section tabs appear: Overview, Tour setup, Player library, Squads & teams, Courses, Rounds & tee times, Matches & pairings, Result entry, Tour itinerary, Admin guide, Settings, Bet Punto, and Coming next.'] },
    { heading: 'Common mistakes / troubleshooting', items: ['The public password is not the Admin PIN and never unlocks Admin.', 'If the Admin PIN does not work, retype it, check for spaces, and confirm the configured admin secret with the app owner.', 'If you cannot leave Admin, use Back to app or the bottom navigation.'] },
  ] },
  { id: 'guide-public-password', title: 'Public password access', summary: 'Manage the separate public tour password without changing Admin PIN security.', blocks: [
    { heading: 'What this is for', items: ['The public password unlocks public pages for guests. It does not unlock Admin and does not grant write access.'] },
    { heading: 'Step-by-step', items: ['Sign in to Admin, open Settings, and find Public password access.', 'Enter the new password and matching confirmation.', 'Leave Force existing public sessions to expire ticked when you need everyone to re-enter the password.', 'Click Save public password.', 'Test in an incognito/private browser so existing browser storage does not hide access problems.'] },
    { heading: 'What should happen', items: ['Public access lasts according to the public access setting/session logic already configured in the app.', 'If forced expiry is enabled, existing public sessions must enter the new password again.'] },
    { heading: 'Troubleshooting', items: ['If the password does not work, check spelling, confirm both fields matched when saved, and test in a private browser.', 'If a configuration error appears, the public access environment/database settings may be missing and may require a deployment or SQL/schema migration check.', 'Do not use the Admin PIN as the public password.'] },
  ] },
  { id: 'guide-tour', title: 'Creating or selecting a tour', summary: 'Create annual tours, select existing tours for editing, and control the current public tour.', blocks: [
    { heading: 'When to create/select', items: ['Create a new tour for a new year or materially separate event.', 'Select an existing tour when correcting rosters, results, itinerary, or archive content for that tour.'] },
    { heading: 'Step-by-step', items: ['Open Tour setup.', 'Click Create new tour or Create next tour.', 'Fill Name, Year, Location, Start date, End date, Status, and Description.', 'Status values are planned, active, complete or archived. Status meanings: planned is being prepared; active is the live/current event; complete is finished but visible historically; archived is historical and should only receive deliberate corrections.', 'Click Create tour or Save tour.', 'Click Set current public only when that is the tour public users should see.'] },
    { heading: 'Public check', items: ['Back to app, open Home, Golf, Tours, Stats and Bet Punto to confirm the correct current public tour is showing.', 'If the wrong tour is current public, public pages can show the wrong rosters, courses, rounds, results, itinerary and Bet Punto markets.'] },
  ] },
  { id: 'guide-2026-format', title: '2026 format setup', summary: 'Use the 2026 helper and verify the generated rounds/prizes.', blocks: [
    { heading: 'Step-by-step', items: ['Open Rounds & tee times.', 'Use the 2026 format helper for the selected tour.', "It creates Sat AM Faldo Course, 4BBB, 18 holes; Sat PM O'Connor Jnr. Course, scramble, 9 holes; Sun Old Course, singles, 18 holes; and Mon Course TBC, 4BBB, 9 holes."] },
    { heading: 'What it does not do', items: ['It does not create Friday golf. Do not assume Friday golf.', 'It does not delete completed data.', 'It does not publish everything unexpectedly; check each Published flag yourself.'] },
    { heading: 'Afterwards', items: ['Review warnings about extra rounds. Extra protected rounds are not removed automatically.', 'Check the Home tour hub, each Golf round card and its Tee sheet, Results and Prizes tabs, then check the Tour information itinerary after saving.'] },
  ] },
  { id: 'guide-courses', title: 'Course guides and scorecards', summary: 'Save official course information against the correct tour and reuse it safely next year.', blocks: [
    { heading: 'Step-by-step', items: ['Select the tour first, then open Courses.', 'Choose a built-in or previous-tour guide to copy, or create a blank course.', 'Enter the official overview, source links, tees and every hole in yards.', 'Only choose official hole-by-hole commentary when the source really provides it.', 'Publish the course, then select that saved guide on the relevant round in Rounds & tee times.'] },
    { heading: 'Archiving and reuse', items: ['Each saved course is a snapshot owned by that tour, so editing next year cannot alter an archived scorecard.', 'For a future tour, use Copy a saved guide, review it against the latest official scorecard, save it to the new tour, then link its rounds.'] },
  ] },
  { id: 'guide-rounds', title: 'Rounds', summary: 'Create, edit, publish, and correct rounds manually.', blocks: [
    { heading: 'Field guide', items: ['Round number controls ordering.', 'Date is the golf date.', 'Session is AM, PM, or TBC and helps avoid mixing Saturday AM and Saturday PM.', 'Course is the public course name.', 'Format is Singles, Better ball, Foursomes, Scramble, or Custom.', 'Holes must be 9 or 18; result dropdown options depend on this.', 'First tee time is the round-level starting point; individual match tee times can be different.', 'Status tracks draft/planned/active/complete; Published controls public visibility.'] },
    { heading: 'Step-by-step', items: ['Open Rounds & tee times.', 'Fill the round form or select an existing round card to edit.', 'Use 9 holes only for true 9-hole sessions; 10 & 8 is not a valid 9-hole result option.', 'Click Save round.', 'Publish only when users should see the round publicly.'] },
    { heading: 'Fixes', items: ['To fix a wrong date/course/holes value, select the round, edit the field, and save.', 'After changing holes, re-check Result entry because the valid result dropdown changes.'] },
  ] },
  { id: 'guide-teams', title: 'Teams and rosters', summary: 'Build teams, attendance, captains, colours, and published rosters.', blocks: [
    { heading: 'Step-by-step', items: ['Open Squads & teams.', 'Create teams with Team name, Colour, Captain, and Sort order.', 'Stored team colours override the blue/red fallback, so fix the saved Colour field if a team colour looks wrong.', 'Tick Attending for each player and click Save attendance changes.', 'Assign attending players to team columns and save the team assignment.', 'Use each team publish control so rosters appear publicly.'] },
    { heading: 'Public check', items: ['Teams appear in Tours > Teams & players and on the Golf Teams tab once teams and rosters are published.', 'If Teams & players says TBC, check attending players, team assignment, team publish flags and current public tour.'] },
  ] },
  { id: 'guide-photos', title: 'Player profiles and photos', summary: 'Maintain global player data and tour-specific public overrides.', blocks: [
    { heading: 'What goes where', items: ['Global player profile fields in Player library are the fallback everywhere.', 'Tour-specific nickname, photo URL, profile/bio, handicap, and notes in Squads & teams override global data for that tour only.', 'Use global data for stable identity details; use tour-specific fields for event-specific photos, nicknames, profiles, and handicaps.'] },
    { heading: 'Photo storage guidance', items: ['Photos are stored outside the form, preferably in Supabase Storage bucket player-photos.', 'Recommended paths: player-photos/global/{player_id}/profile.{ext} and player-photos/tours/{tour_id}/{player_id}/profile.{ext}.', 'Do not use player names as file paths because names can change.', 'Direct upload is not currently built into Admin; upload separately and paste a public URL/path into Photo URL or Tour photo URL.'] },
    { heading: 'Troubleshooting', items: ['If a photo is missing or broken, check the URL, bucket permissions, ID-based path, and whether a tour-specific override is replacing the global photo.', 'Broken or missing photos fall back to initials on public profile cards.'] },
  ] },
  { id: 'guide-pairings', title: 'Pairings and tee times', summary: 'Create matches, assign sides/players, set tee times, and publish pairings.', blocks: [
    { heading: 'Step-by-step', items: ['Open Matches & pairings.', 'Choose the round carefully, especially Sat AM versus Sat PM.', 'Set Match number, Format, Status, teams/sides, players, Tee time, Published, and Notes.', 'Do not reuse selected players in the same round unless you are deliberately correcting data; the UI warns/filters to help avoid duplicates.', 'Click Save match.', 'Use sequential tee times when draft/planned matches should receive individual tee times from a first tee time and interval.', 'Publish matches when pairings should appear publicly.'] },
    { heading: 'Round vs match tee times', items: ['Round first tee time is the headline start time for a round.', 'Match tee time is the actual tee time for an individual pairing and is what users need for pairings.'] },
    { heading: 'Troubleshooting', items: ['Golf is round-led: choose a round card, then use Tee sheet, Results or Prizes. If pairings do not show, check round Published, match Published, current public tour, and public password access.', 'If tee times show TBC, enter match tee times or apply sequential tee times and save.'] },
  ] },
  { id: 'guide-results', title: 'Match results', summary: 'Enter controlled matchplay results without changing scoring logic.', blocks: [
    { heading: 'Step-by-step', items: ['Open Result entry.', 'Select the Round and Match.', 'Choose the controlled result from the dropdown.', 'AS means all square/halved.', 'Set Published if the completed result should be public and save/submit the result.', 'To clear or correct a result, select the match again, choose the corrected result/status, add a correction reason where prompted, and save.'] },
    { heading: 'Status and points', items: ['Match statuses include planned, active, complete, and void.', 'Wins/losses update points from the saved result; AS splits/halves points according to the existing scoring rules.', 'Valid 18-hole and 9-hole dropdown options differ. 10 & 8 is valid for 18 holes but not for 9 holes.'] },
    { heading: 'Troubleshooting', items: ['If the homepage score does not update, check the match status, published state, selected round holes, current public tour, and refresh public data.', 'If the dropdown has wrong options, check whether the round is saved as 9 or 18 holes.'] },
  ] },
  { id: 'guide-secondary-prizes', title: 'Secondary prize results', summary: 'Record the playing winner and settle its linked Bet Punto market in one action.', blocks: [
    { heading: 'What this is for', items: ['Playing prizes are Highest Stableford score for an individual round or Lowest scramble gross for a team round.', 'They remain separate from the main matchplay score, but a published winner automatically settles the matching Bet Punto market.'] },
    { heading: 'Step-by-step', items: ['Open the Live results desk or Result entry.', 'Enter the winning player/team and score.', 'Publish the prize result after the fixed first-tee market close.', 'The app assigns any automatic defaults, calculates pooled payouts and updates the public tour tally.', 'To correct a winner or score, save the corrected prize result; the linked market is recalculated.'] },
    { heading: 'Troubleshooting', items: ['If a prize winner does not appear publicly, check the selected round, published flag, current public tour, and whether the winner/player/team is valid for that tour.'] },
  ] },
  { id: 'guide-bet-punto', title: 'Bet Punto', summary: 'Prepare the two playing markets, then let first-tee defaults and prize settlement run automatically.', blocks: [
    { heading: 'What Bet Punto is / is not', items: ['Bet Punto tracks friendly picks, stakes, pooled payouts and each player’s running net position.', 'It has no wallet, no payment handling and no actual money transfer; final payment remains offline.'] },
    { heading: 'Step-by-step', items: ['Open Bet Punto and build missing daily markets.', 'Non-scramble rounds use Highest Stableford; scramble rounds use Lowest scramble gross.', 'The backend fixes the market close to the selected tour timezone and earliest tee time for that round.', 'Users choose £5 increments and must reach £10 for the playing day.', 'At the first tee, the app closes the market and automatically tops any shortfall to £10 onto the player themselves or their own scramble team.', 'Enter and publish the playing winner in the Live results desk; the app settles the pool and updates the public leaderboard.', 'Use manual admin bet entry only for a deliberate correction. Money movement remains outside the app.'] },
    { heading: 'Troubleshooting', items: ['If a market does not show publicly, check market status, linked round/match publish state, close time, current public tour, and visibility warnings.', 'If users cannot submit a bet or edit before close, check market status, close time, required bettor name, option availability, and public password access.', 'If duplicate markets exist, void/archive the unwanted one or delete only safe draft data.'] },
  ] },
  { id: 'guide-info', title: 'Tour itinerary', summary: 'Maintain one practical schedule for where to be, when, and what to wear.', blocks: [
    { heading: 'Single-source rules', items: ['Travel, accommodation and dinners are entered once in Tour itinerary.', 'Golf course and first tee time come directly from Rounds & tee times and are never duplicated as manual schedule rows.', 'Daily shirt colours are saved by tour, date and team.', 'Old handbook and miscellaneous schedule records remain preserved but are hidden from the live itinerary.'] },
    { heading: 'Step-by-step', items: ['Open Tour itinerary.', 'Add each flight or transfer with its date, fixed time, location and essential details.', 'Add the accommodation once with basic location or booking information.', 'Add each dinner with its date, time and location.', 'Add each team shirt colour against the relevant date.', 'Edit golf in Rounds & tee times; the itinerary updates automatically.'] },
    { heading: 'Troubleshooting', items: ['If golf is wrong, correct the selected tour’s round rather than adding another itinerary row.', 'If a travel, stay or dinner row is missing, check its type and selected tour.', 'If shirts are missing, check both teams exist for the selected tour and each colour has the correct date.'] },
  ] },
  { id: 'guide-publishing', title: 'Publishing and visibility', summary: 'Understand why Admin data may not be visible publicly.', blocks: [
    { heading: 'Rules', items: ['Public password access only gets a user through the gate; it does not publish private content.', 'Users see content for the current public tour only.', 'Rounds, teams/roster, matches, results, secondary prize results and itinerary content must be visible according to their own controls.'] },
    { heading: 'Troubleshooting', items: ['If something appears in Admin but not public, check current public tour, publish flags, status, linked round/match, and refresh.', 'If something appears public too early, unpublish the item or return it to draft/private status.'] },
  ] },
  { id: 'guide-archive', title: 'Archiving and next year setup', summary: 'Finish a tour, preserve history, and prepare next year.', blocks: [
    { heading: 'Step-by-step', items: ['In Tour setup, set a finished tour to complete when the event is done.', 'Use archived for older tours that should remain historical.', 'Do not delete completed courses, matches, results, rosters, itinerary, betting or player snapshots just to start a new year.', 'Create next year’s tour with Create next tour, copy/review its course guides, then build rounds, rosters, itinerary and Bet Punto against that new selected tour.'] },
    { heading: 'What remains / resets', items: ['Historical courses, teams, rounds, matches, results, itinerary, shirt colours, prize results, betting and tour-specific player profiles remain with that tour.', 'The next tour starts with its own itinerary, courses and Bet Punto records; only reusable guide data is copied deliberately.', 'Tour-specific snapshots protect historic tours from later edits.'] },
  ] },
  { id: 'guide-troubleshooting', title: 'Troubleshooting centre', summary: 'Decision-tree checks for common Admin and public issues.', blocks: [
    { heading: 'Decision tree', items: ['Public page shows wrong tour: check Tour setup > current public tour, then Back to app and refresh.', 'Public page still asks for password: enter the public password, check expiry/force-expire settings, and test in private browsing.', 'Public password does not work: save it again in Settings, confirm it is not the Admin PIN, and check configuration if an error appears.', 'Admin PIN does not work: confirm the Admin PIN/secret with the owner; the public password will not work here.', 'Schema/cache/migration error: refresh, check Supabase errors, and run the required SQL/schema migration or schema cache reload when fields/functions are missing.', 'Published round does not show: check current public tour, round Published, round status/date, and public refresh.', 'Published teams do not show: check team Published, attending players, team assignments, and current public tour.', 'Matches do not show: check round Published, match Published, match status, and selected public Golf tab.', 'Tee times do not show: check match Tee time values; round first tee time alone may not fill every pairing.', 'Score does not update: check result entry, match status complete, published flags, and valid holes/result options.', 'Result dropdown has wrong options or 9-hole result options look wrong: check the saved round Holes value.', 'Bet Punto market does not show: check market status, close time, linked round/match visibility, and visibility warnings.', 'Bet Punto bet cannot be submitted: check market is open, has options, user selected a bettor name, and public password access is valid.', 'User cannot edit their bet: check whether the market is closed/settled/void or the close time has passed.', 'Photo does not show: check Photo URL/Tour photo URL, player-photos bucket access, and ID-based path.', 'Itinerary shows the wrong golf: correct the round; golf schedule rows are generated automatically.', 'Travel, stay or dinner is missing: open Tour itinerary and check the selected tour, type and date.', 'Team colours look wrong: edit the saved team Colour for branding or the dated shirt colour in Tour itinerary.', 'Logo looks wrong: confirm the brand asset path and browser cache; this is not an Admin data setting.', 'Mobile layout looks broken: test the same section at mobile width and report the exact card/tab; avoid public redesign changes in Admin fixes.', 'Admin page has no navigation / cannot leave Admin: use Back to app, bottom navigation, or End admin session; if absent, this PR has regressed.'] },
  ] },
];
...
function emptyTourForm(tour?: Tour): TourForm {
  return { id: tour?.id, name: tour?.name ?? '', year: tour?.year ? String(tour.year) : '', location: tour?.location ?? '', startDate: tour?.startDate ?? '', endDate: tour?.endDate ?? '', timezone: tour?.timezone ?? 'Europe/London', status: tour?.status ?? 'planned', description: tour?.description ?? '' };
}
...
function emptyRoundForm(round?: Round): RoundForm {
  ...
  return { id: round?.id, roundNumber: round?.roundNumber ? String(round.roundNumber) : '1', name: round?.name ?? 'Round 1', roundDate: round?.roundDate ?? '', session: noteParts.session, courseId: round?.courseId ?? '', courseName: round?.courseName ?? '', teeTime: round?.teeTime ?? '', format: formatByLabel.get((round?.formatLabel ?? '').toLowerCase()) ?? 'singles', holes: String(round?.holes ?? 18) as '18' | '9', notes: noteParts.notes, status: round?.status ?? 'draft' };
}
...
export function Admin() {
  ...
  const [adminMode, setAdminMode] = useState<'live' | 'setup'>('live');
  ...
  const [roundPrizeForm, setRoundPrizeForm] = useState<RoundPrizeForm>(emptyRoundPrizeForm);
  ...
  const [itineraryForm, setItineraryForm] = useState<ItineraryForm>(emptyItineraryForm);
  ...
  const [publicPasswordForm, setPublicPasswordForm] = useState({ password: '', confirmPassword: '', forceExpire: true });
  ...
  const resultOptions = matchplayResultOptionsForHoles(resultRound?.holes ?? 18);
  ...
  const navigateToPublicApp = () => { window.history.pushState(null, '', '/'); window.dispatchEvent(new PopStateEvent('popstate')); window.scrollTo({ top: 0, behavior: 'smooth' }); };
  ...
  const submitRound = (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); if (!selectedTour) return; void runSave('round', 'Round saved. Its daily Bet Punto market was prepared when eligible options were available.', async () => { const response = await saveRound({ ...roundForm, tourId: selectedTour.id, roundNumber: Number(roundForm.roundNumber), roundDate: roundForm.roundDate || null, holes: Number(roundForm.holes) as 9 | 18, courseId: roundForm.courseId || null, courseName: roundForm.courseName || null, teeTime: roundForm.teeTime || null, notes: combineRoundNotes(roundForm.session, roundForm.notes) }); await createDailyMarketForRound(response.round); return selectedTour.id; }); };
  ...
  const planRounds = () => {
    ...
    void runSave('plan-rounds', 'Round setup saved. The correct daily markets were prepared when eligible options were available.', async () => {
  ...
  const setSimpleMarketKind = (kind: 'player_winner' | 'team_winner') => {
    ...
    replaceBetOptions({
      ...betMarketForm,
      title: betMarketForm.title || `${betMarketForm.roundId ? roundBetLabel(betMarketForm.roundId) : 'Round'} lowest scramble gross`,
      description: betMarketForm.description || 'Pick the team with the lowest gross score in the scramble.',
  ...
  const addMissingDefaultPicks = (market: BetMarket) => {
    if (!selectedTour) return;
    void runSave(`default-bets-${market.id}`, 'Automatic first-tee defaults checked.', async () => selectedTour.id);
  };

  const applyRoundTeamScoreOptions = () => {
    if (!betMarketForm.roundId) {
      setSaveState('bet-market', { saving: false, error: 'Choose a scramble round before building team options.' });
      return;
    }
    ...
    replaceBetOptions({
      ...betMarketForm,
      title: betMarketForm.title || `${roundBetLabel(betMarketForm.roundId)} lowest scramble gross`,
      description: betMarketForm.description || 'Pick the team with the lowest gross score in the scramble.',
      marketType: 'team_result',
      status: 'draft',
      required: true,
  ...
  const createDailyMarketForRound = async (round: Round) => {
    ...
    const expectedType: BetMarket['marketType'] = isScramble ? 'team_result' : 'player_performance';
    if (adminData.betMarkets.some((market) => market.roundId === round.id && market.marketType === expectedType && market.status !== 'void')) return false;
    const teamIdsFromRound = [...new Set(adminData.matches.filter((match) => match.roundId === round.id).flatMap((match) => [match.sideATeamId, match.sideBTeamId]).filter(Boolean))];
    const eligibleTeamIds = teamIdsFromRound.length > 0 ? teamIdsFromRound : adminData.tourTeams.map((team) => team.id);
    const options = isScramble
      ? eligibleTeamIds.map((teamId, index) => ({ ...emptyBetOptionForm(index + 1), label: teamsById.get(teamId)?.name ?? `Team ${index + 1}`, linkedTeamId: teamId }))
    ...
    await saveBetMarket({
      ...
      roundId: round.id,
      matchId: null,
      title: isScramble ? `${roundBetLabelFromRound(round)} lowest scramble gross` : `${roundBetLabelFromRound(round)} highest Stableford`,
      description: isScramble ? 'Pick the team with the lowest gross score in the scramble.' : 'Pick the player with the highest Stableford score for this round/day.',
  ...
  const createDailyMarketsForRounds = () => {
    ...
    void runSave('bet-market', 'Missing daily markets created: highest Stableford, or lowest gross for scramble rounds.', async () => {
      for (const round of adminData.rounds) await createDailyMarketForRound(round);
    });
  };
  ...
  const editRoundPrize = (prize: RoundPrizeResult) => setRoundPrizeForm({ id: prize.id, roundId: prize.roundId, prizeType: prize.prizeType, title: prize.title, winnerPlayerId: prize.winnerPlayerId ?? '', winnerTeamId: prize.winnerTeamId ?? '', winningScoreText: prize.winningScoreText ?? '', scoreValue: prize.scoreValue === undefined ? '' : String(prize.scoreValue), scoreUnit: prize.scoreUnit ?? '', notes: prize.notes ?? '', linkedBetMarketId: prize.linkedBetMarketId ?? '', published: prize.published });

  const submitRoundPrize = (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    if (!selectedTour) return;
    void runSave('round-prize', 'Secondary prize result saved.', async () => {
      await saveRoundPrizeResult({ ...roundPrizeForm, tourId: selectedTour.id, winnerPlayerId: roundPrizeForm.winnerPlayerId || null, winnerTeamId: roundPrizeForm.winnerTeamId || null, linkedBetMarketId: roundPrizeForm.linkedBetMarketId || null, winningScoreText: roundPrizeForm.winningScoreText || null, scoreValue: roundPrizeForm.scoreValue ? Number(roundPrizeForm.scoreValue) : null, scoreUnit: roundPrizeForm.scoreUnit || null, notes: roundPrizeForm.notes || null });
  ...
  const removeRoundPrize = (prize: RoundPrizeResult) => {
    if (!selectedTour) return;
    void runSave(`delete-round-prize-${prize.id}`, 'Secondary prize result deleted.', async () => {
  ...
  const addDefaultRoundPrizes = () => {
    if (!selectedTour) return;
    void runSave('default-round-prizes', 'Default secondary prize slots created.', async () => {
  ...
  const refreshPublicAccessStatus = async () => {
    try {
      ...
      setPublicAccessStatus(`${settings.configured ? 'Custom password configured' : 'Using default password'} · session version ${settings.sessionVersion}`);
    } catch (error) {
      setPublicAccessStatus(error instanceof Error ? error.message : 'Public access settings unavailable.');
    }
  };

  const savePublicPassword = (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    if (publicPasswordForm.password !== publicPasswordForm.confirmPassword) { setSaveState('public-access-settings', { saving: false, error: 'Passwords do not match.' }); return; }
    void runSave('public-access-settings', 'Public app password updated.', async () => {
      await savePublicAccessSettings({ password: publicPasswordForm.password, forceExpire: publicPasswordForm.forceExpire });
      setPublicPasswordForm({ password: '', confirmPassword: '', forceExpire: true });
  ...
  const applyFormatTemplate2026 = () => {
    if (!selectedTour) return;
    if (!window.confirm('Apply the agreed 2026 format template to this tour? This updates or creates planned rounds, but does not change the tour itinerary, matches or results.')) return;
    void runSave('apply-2026-template', '2026 format template applied.', async () => { const result = await apply2026FormatTemplate({ tourId: selectedTour.id }); if (result.warnings?.length) throw new Error(`2026 format template applied, but review required: ${result.warnings.join(' ')}`); return selectedTour.id; });
  };

  const addPrizeSlots2026 = () => {
    if (!selectedTour) return;
    void runSave('default-round-prizes-2026', '2026 secondary prize slots created.', async () => { await createDefaultRoundPrizeResults({ tourId: selectedTour.id, template2026: true }); return selectedTour.id; });
  };
  ...
  const submitItineraryItem = (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    if (!selectedTour) return;
    const sortOrder = Number(itineraryForm.sortOrder);
    if (!itineraryForm.activity.trim()) {
      setSaveState('itinerary-item', { saving: false, error: 'Add an activity.' });
      return;
    }
    if (!Number.isFinite(sortOrder)) {
      setSaveState('itinerary-item', { saving: false, error: 'Sort order must be numeric.' });
      return;
    }
  ...
  const removeItineraryItem = (itemId: string) => {
    if (!selectedTour || !window.confirm('Delete this itinerary item?')) return;
    void runSave(`delete-itinerary-${itemId}`, 'Itinerary item deleted.', async () => {
      await deleteItineraryItem({ id: itemId, tourId: selectedTour.id });
      return selectedTour.id;
    });
  };
  ...
  const buildDraftItinerary = () => {
    ...
    void runSave('build-itinerary', 'Draft itinerary built from actual rounds.', async () => {
      for (const draft of missing) await saveItineraryItem({ tourId: selectedTour.id, ...draft });
      return selectedTour.id;
    });
  };
  ...
  return <div className="page-stack admin-page">
    <section className="page-title admin-title-bar"><div><p className="eyebrow">Live setup management · Admin mode</p><h2>Admin</h2><p>PIN-protected tools for live tour lifecycle, squads, rounds and draft/published pairings. Result entry, handicaps and Bet Punto tracking stay behind this admin session.</p></div><div className="admin-title-actions"><button className="admin-secondary-button neutral" type="button" onClick={navigateToPublicApp}>Back to app</button>{storedSession ? <button className="admin-secondary-button" type="button" onClick={handleLogout}>End admin session</button> : null}</div></section>
    <section className="card admin-login-panel"><div><p className="eyebrow">Admin PIN session</p><h3>{storedSession ? 'Admin session active' : 'Sign in to unlock admin writes'}</h3><p>{storedSession ? `This browser has a short-lived admin session until ${expiresAtLabel}.` : 'Enter the shared admin PIN to create a short-lived browser session. The optional label is only for admin context, not a user account.'}</p></div>{storedSession ? <div className="admin-login-actions"><button className="admin-secondary-button neutral" type="button" onClick={navigateToPublicApp}>Back to app</button><button className="admin-secondary-button" type="button" onClick={handleLogout}>End admin session</button></div> : <form className="admin-login-form" onSubmit={handleLogin}><label>Admin label<input value={actorLabel} onChange={(event) => setActorLabel(event.target.value)} placeholder="Optional admin label" /></label><label>Shared PIN<input value={pin} onChange={(event) => setPin(event.target.value)} inputMode="numeric" type="password" autoComplete="current-password" /></label>{loginError ? <p className="form-error">{loginError}</p> : null}<button type="submit" disabled={loginState === 'submitting'}>{loginState === 'submitting' ? 'Signing in…' : 'Create admin session'}</button></form>}</section>

    {storedSession ? <>
      <nav className="admin-mode-switch" aria-label="Admin mode"><button type="button" className={adminMode === 'live' ? 'selected' : ''} onClick={() => setAdminMode('live')}><strong>Live desk</strong><small>Results, winner, publish</small></button><button type="button" className={adminMode === 'setup' ? 'selected' : ''} onClick={() => setAdminMode('setup')}><strong>Setup</strong><small>Tour, teams, rounds, pairings</small></button></nav>
      {adminMode === 'setup' ? <nav className="admin-section-nav" aria-label="Admin setup sections">{tabs.map((tab) => <button className={`pill ${activeTab === tab ? 'selected' : ''}`} type="button" onClick={() => setActiveTab(tab)} key={tab}>{tab}</button>)}</nav> : null}
