...
export function Dashboard() {
  ...
  const recentResults = matches.filter((match) => match.status === 'complete').slice(0, 3);
  return <div className="page-stack"><section className="hero card"><p className="eyebrow">Current tour</p><h2>{tour.name}</h2><p>{tour.description}</p></section>
    ...
    <section><h2>Recent results</h2>{recentResults.map((match) => <MatchCard key={match.id} match={match} participants={matchParticipants.filter((p) => p.matchId === match.id)} players={players} teams={tourTeams} />)}</section>
    <section className="quick-links card"><h2>Quick links</h2><p>Use the bottom navigation for stats, matches, players and betting.</p></section>
  </div>;
}
