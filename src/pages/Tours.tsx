...
function TourDetail({ tour, data, dataForStats, onBack }: { tour: Tour; data: Omit<PublicAdvancedStatsResponse, 'source'>; dataForStats: AdvancedStatsData; onBack: () => void }) {
  ...
  return <div className="page-stack tours-page selected-tour-page">
    ...
    <section className="selected-tour-hero card">
      ...
      <div className="tour-detail-score">{scores.length > 0 ? scores.map((row, index) => <span className="team-score-pill" key={row.team.id} style={{ '--team-colour': normalizeTeamColour(row.team.colour, index) } as CSSProperties}>{row.team.name} <b>{formatPoints(row.points)}</b></span>) : <span>Score TBC</span>}</div>
    </section>
...
function TourTeams({ teams, data }: { teams: TourTeam[]; data: Omit<PublicAdvancedStatsResponse, 'source'> }) {
  const tourPlayerByKey = new Map((data.tourPlayers ?? []).map((tourPlayer) => [`${tourPlayer.tourId}:${tourPlayer.playerId}`, tourPlayer]));
  ...
  return <section className="tour-detail-section"><h3>Teams</h3>{teams.length === 0 ? <p className="card">Teams TBC</p> : <div className="team-card-grid">{teams.map((team, index) => {
    ...
    const members = data.tourTeamMembers.filter((member) => member.teamId === team.id).map((member) => { const player = playerById.get(member.playerId); return player ? tourDisplayPlayer(player, tourPlayerByKey.get(`${team.tourId}:${member.playerId}`)) : undefined; }).filter(Boolean);
