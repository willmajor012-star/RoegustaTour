...
import { fetchPublicTourInfo } from '../lib/publicApi';
import { usePublicData } from '../lib/usePublicData';
import { localTourInfoFallback } from '../lib/localFallbackData';

export function TourInfo() {
  const { data, loading, error, source } = usePublicData(fetchPublicTourInfo, {
    localFallback: localTourInfoFallback,
    onErrorMessage: 'Live tour handbook data is unavailable, so local demo data is shown instead.',
  });
  const activeData = data ?? localTourInfoFallback;
  ...
  return <div className="page-stack"><section className="page-title"><p className="eyebrow">Tour handbook</p><h2>{tour?.name ?? 'Roegusta Tour'}</h2></section>
    ...
    <section className="card"><h3>Details</h3><p>{tour?.location ?? 'Location TBC'}</p><p>{formatDate(tour?.startDate)} — {formatDate(tour?.endDate)}</p>{tour?.description && <p>{tour.description}</p>}</section>
