...
export function TourInfo() {
  ...
  return <div className="page-stack handbook-page"><section className="page-title premium-title"><p className="eyebrow">Tour handbook</p><h2>{tour?.name ?? 'Handbook'}</h2></section>
    ...
    <section className="card"><div className="section-heading"><div><p className="eyebrow">Course guide</p><h2>Rounds</h2></div></div>{activeData.rounds.length === 0 ? <p>Round details will appear once added.</p> : <div className="premium-list">{activeData.rounds.map((round, index) => <div className="premium-list-row" key={round.id}><strong>{formatRoundDisplayName(round, index)}</strong><span>{round.courseName ?? 'Course TBC'} · {normalizeTeeTime(round.teeTime) ?? 'TBC'}</span></div>)}</div>}</section>
