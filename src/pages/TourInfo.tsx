...
import { formatRoundContextLabel, formatRoundDisplayName, formatTeeTimeDisplay } from '../lib/display';
import { fetchPublicTourInfo, type PublicTourInfoResponse } from '../lib/publicApi';
import { usePublicData } from '../lib/usePublicData';

const emptyTourInfo: Omit<PublicTourInfoResponse, 'source'> = { rounds: [], handbookSections: [], itineraryItems: [], teamDayKit: [], tourTeams: [], roundPrizeResults: [] };
...
export function TourInfo() {
  ...
  return <div className="page-stack handbook-page"><section className="page-title premium-title"><p className="eyebrow">Tour handbook</p><h2>{tour?.name ?? 'Handbook'}</h2></section>
    ...
    <section className="card"><div className="section-heading"><div><p className="eyebrow">Schedule</p><h2>Itinerary</h2></div></div>{activeData.itineraryItems.length === 0 ? <p>Itinerary TBC.</p> : <div className="timeline-list">{activeData.itineraryItems.map((item) => <article className="timeline-item" key={item.id}><span>{item.dayLabel ?? formatDate(item.itemDate)}</span><div><strong>{item.timeLabel ? `${item.timeLabel} · ` : ''}{item.activity}{item.isPlaceholder && !/tbc/i.test(`${item.timeLabel ?? ''} ${item.activity}`) ? ' · TBC' : ''}</strong>{item.location && <p>{item.location}</p>}{item.notes && <p>{item.notes}</p>}</div></article>)}</div>}</section>
