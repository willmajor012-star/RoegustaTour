...
function itemIsActive(path: string, currentPath: string) {
  if (path === '/') return currentPath === '/' || currentPath === '/score';
  if (path === '/tours') return currentPath === '/tours' || currentPath === '/teams' || currentPath === '/info' || currentPath === '/courses' || currentPath.startsWith('/courses/');
...
export function BottomNav({ currentPath, onNavigate }: Props) {
  return <nav className="bottom-nav" aria-label="Primary navigation">
    {navigationItems.map(({ path, label, icon }) => {
      const isActive = itemIsActive(path, currentPath);
      return <button key={path} type="button" className={isActive ? 'active' : ''} aria-current={isActive ? 'page' : undefined} onClick={() => onNavigate(path)}>
        <NavIcon name={icon} />
        <span>{label}</span>
      </button>;
    })}
