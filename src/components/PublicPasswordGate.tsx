...
export function PublicPasswordGate({ children, isAdminRoute }: Props) {
  ...
  return <div className="public-password-screen"><section className="public-password-card card"><span className="brand-logo-roundel public-password-logo"><img className="brand-logo" src="/brand/roegusta-logo-mark.png" alt="Roegusta Tour mark" /></span><p className="eyebrow">Private golf tour</p><h1>Roegusta Tour</h1><p>Enter the tour password to continue.</p><form onSubmit={submit}><label>Password<input type="password" autoComplete="current-password" value={password} onChange={(event) => setPassword(event.target.value)} /></label>{error ? <p className="form-error">{error}</p> : null}<button type="submit" disabled={submitting}>{submitting ? 'Checking…' : 'Continue'}</button></form><small>Public access is separate from Admin PIN access.</small></section></div>;
}
