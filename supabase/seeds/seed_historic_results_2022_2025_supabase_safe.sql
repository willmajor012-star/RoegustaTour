...
-- Supabase-safe variant: uses real one-off staging tables instead of TEMP tables,
-- because the SQL editor can lose TEMP table scope if statements are executed separately.
...
drop table if exists public._rt_historic_players;
create table public._rt_historic_players (display_name text primary key);
insert into public._rt_historic_players (display_name) values
  ('Adam Musikant'),
  ('Alex Tonge'),
  ('Ben Walmsley'),
  ('Brian Crotty'),
...
select hp.display_name,
       ...
       true
from public._rt_historic_players hp
where not exists (
  select 1 from players p where lower(btrim(p.display_name)) = lower(btrim(hp.display_name))
);

drop table if exists public._rt_historic_tours;
create table public._rt_historic_tours (year integer primary key, name text, status text);
insert into public._rt_historic_tours (year, name, status) values
...
from public._rt_historic_tours
...
drop table if exists public._rt_historic_team_map;
create table public._rt_historic_team_map (
  year integer,
  side text,
  team_name text,
...
insert into public._rt_historic_team_map (year, side, team_name, final_points, result_status, confirmation_note) values
...
from public._rt_historic_team_map htm
join tours t on t.year = htm.year
on conflict (tour_id, name) do update set
  colour = excluded.colour,
  sort_order = excluded.sort_order;

drop table if exists public._rt_historic_rounds;
create table public._rt_historic_rounds (
  year integer,
...
insert into public._rt_historic_rounds (year, round_number, format, format_label, course, match_count, side_a_points, side_b_points) values
...
from public._rt_historic_rounds hr
...
drop table if exists public._rt_historic_matches;
create table public._rt_historic_matches (
  year integer,
  round_number integer,
...
insert into public._rt_historic_matches (year, round_number, match_number, format, format_label, course, match_type, side_a_team, side_a_players, result_text, side_b_team, side_b_players, points_available, points_side_a, points_side_b, winning_side) values
...
from public._rt_historic_matches hm
...
drop table if exists public._rt_historic_match_players;
create table public._rt_historic_match_players as
...
from public._rt_historic_matches hm, regexp_split_to_table(hm.side_a_players, ',') as player_name
union all
select hm.year, hm.round_number, hm.match_number, hm.format, hm.winning_side,
       hm.points_side_a, hm.points_side_b, 'B'::text as side, hm.side_b_team as team_name, btrim(player_name) as display_name
from public._rt_historic_matches hm, regexp_split_to_table(hm.side_b_players, ',') as player_name;

insert into tour_players (tour_id, player_id, attending)
select distinct t.id, p.id, true
from public._rt_historic_match_players hmp
...
-- Clean up one-off staging tables.
drop table if exists public._rt_historic_match_players;
drop table if exists public._rt_historic_matches;
drop table if exists public._rt_historic_rounds;
drop table if exists public._rt_historic_team_map;
